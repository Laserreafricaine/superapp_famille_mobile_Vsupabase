(() => {
  ...
  function crCoursesScreen(){ const allItems=(data.shopping||[]).filter(x=>!statusIsHidden(x)); const on=batchOn();
    ...
    return crUnivers('courses', `<div class="cr-addcard"><div class="cr-add-lbl">🛒 Ajouter une course</div><div class="cr-quick"><input id="crQadd" placeholder="Ex : Tomates, Lait, Pâtes…" oninput="SuperApp.crQHint()" onkeydown="if(event.key==='Enter'){event.preventDefault();SuperApp.crQuickAdd();}"><button onclick="SuperApp.crQuickAdd()">＋ Ajouter</button></div><div class="cr-qhint empty" id="crQhint">Rangé automatiquement dans le bon rayon…</div></div>
  ...
  function crStockHint(){ const inp=document.getElementById('crSadd'); const h=document.getElementById('crShint'); if(!inp||!h) return; const v=(inp.value||'').trim(); if(v){ const r=crRayonByKey(crClassify(v)); h.classList.remove('empty'); h.textContent='→ rangé dans '+r.ico+' '+r.name+' · niveau Bon'; } else { h.classList.add('empty'); h.textContent='Rangé automatiquement · niveau « Bon » par défaut…'; } }
  function crQHint(){ const inp=document.getElementById('crQadd'); const h=document.getElementById('crQhint'); if(!inp||!h) return; const v=(inp.value||'').trim(); if(v){ const r=crRayonByKey(crClassify(v)); h.classList.remove('empty'); h.textContent='→ rangé dans '+r.ico+' '+r.name; } else { h.classList.add('empty'); h.textContent='Rangé automatiquement dans le bon rayon…'; } }
  ...
  function crStockRename(id){ const x=(data.stock||[]).find(s=>s.id===id); if(!x) return; const v=prompt('Modifier le produit',x.title); if(v&&v.trim()){ x.title=v.trim(); save(); } crCloseSheet(); render(); }
  ...
  function crEmptyCell(day,slot){ if(state.mealArmed){ crPlaceMeal(day,slot); return; }
    ...
    crOpenSheet(ex?'Modifier le repas':'Ajouter un repas', dayN+' · '+sl, `${cur}${habs.length?`<div class="cr-sheet-lab">${ex?'Remplacer par un habituel':'Tes plats habituels'}</div><div class="cr-pickhabs">${chips}</div>`:''}<div class="cr-sheet-lab">…ou tape un plat</div><div class="cr-typezone"><input id="crMealInput" placeholder="Ex : Couscous, Gratin…" onkeydown="if(event.key==='Enter'){event.preventDefault();SuperApp.crTypeMeal(${day},'${slot}');}"><button onclick="SuperApp.crTypeMeal(${day},'${slot}')">＋</button></div>${moreActs}`);
    setTimeout(()=>{ const i=document.getElementById('crMealInput'); if(i){ try{i.focus();}catch(e){} } },80); }
